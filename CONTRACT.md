...
| L2.3 Review bots | Optional. Every workflow under `.github/workflows/` that triggers on `pull_request` or `pull_request_target` and has a step with a `prompt` input is a review workflow. One that triggers on `pull_request_target` fails outright. Otherwise it must declare `permissions` (at the top level, or on every job) that do not grant `contents: write`; a workflow with no `permissions` declaration fails, because the default token may write. Every job needs a timeout, and the prompt must tell the model to read `AGENTS.md`. None present reports n/a. `adopt` writes nothing for this item; `agentify review --provider <name>` writes `src/agentify/templates/review.yml` rendered for that provider (and `src/agentify/templates/docs-review.yml` with `--docs-review`) into `.github/workflows/`, then names the repository secret to create. | `src/agentify/checks/l2_3_review.py::check` |
