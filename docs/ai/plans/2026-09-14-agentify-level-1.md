...
def test_python_templates_are_lint_and_format_clean(tmp_path):
    ...
    # Pin the set so a renamed or deleted template cannot turn this into a vacuous pass.
    assert [t.name for t in templates] == ["test_contract.py.tmpl", "test_docs_references.py.tmpl"]
...
`pyproject.toml`), `raised` (the level in `.agentify.toml`), or `skipped`. A
file it reports as `exists` is yours; if it lacks something the contract
